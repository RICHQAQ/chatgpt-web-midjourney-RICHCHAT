...
import { fetchCost } from '@/api'//增加计费接口（未来修改成更好的）
...
const goFinish= (model ?: any,message ?: any[],text ?:any)=>{
    ...
    // console.log(message,model,text)
    // console.log(tokenJson.data)
		if(model != null)
		{
			const Cost = async () => {
			const response: any = await fetchCost<any>(tokenJson.data,message,text,model);
			// console.log(response);
			if(response.status == "Success" && response.message != null)
				ms.info( "花费" + response.message);
			}
...
const submit= (model:string, message:any[] ,  opt?:any )=>{
    ...
    controller.value = new AbortController();
        ...
        //controller.signal
            ...
            // 这里进行后续处理
            }).then(()=>goFinish(model,message,text) ).catch(e=>{
